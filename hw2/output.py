...
from load import MAX_LENGTH, loadPrepareData_test, Voc
from model import *
...
def evaluateRandomly(encoder, decoder, voc, pairs, reverse, beam_size, n=9999):
    input_sentences = []
    output_sentences = []
    for _ in range(n):
        pair = pairs[_]
        ...
        #print("=============================================================")
        if reverse:
            pass#print('>', " ".join(reversed([word for word in pair[0]])))
        else:
            pass#print('>', " ".join([word for word in pair[0]]))
        if beam_size == 1:
...
def runTest(n_layers, hidden_size, reverse, modelFile, beam_size, inp, corpus):
    ...
    if inp:
        ...
        input_sen, output_sen = evaluateRandomly(encoder, decoder, voc, pairs, reverse, beam_size)
    return input_sen, output_sen

def Filename(filename, test=False):
...
n_layers, hidden_size, reverse = Filename('./save/model/conversation/2-2_128/10000_backup_bidir_model.tar')
modelFile = './save/model/conversation/2-2_128/10000_backup_bidir_model.tar'
corpus = './test_input.txt'
beam_size = 1
inp = False

input_sen, output_sen = runTest(n_layers, hidden_size, reverse, modelFile, beam_size, inp, corpus)
with open('./input__.txt', 'w') as f:
    [f.write(line + '\n') for line in input_sen]
with open('./output__.txt', 'w') as f:
    [f.write("<EOS>\n") if line == "" else f.write(line + '\n') for line in output_sen]
